...
void Halfedge_Mesh::triangulate() {
	...
	for (FaceRef f = faces.begin(); f != faces.end(); f++) {
		...
		auto previousNewHalfedge = h;
		auto previousNewFace = f;
		for (int i = 2; i < n - 1; i++) {
			auto current = halfedges[i];
			...
			halfedges[i - 1]->next = newHalfedgeTwin;

			newHalfedge->next = current;
			newHalfedge->twin = newHalfedgeTwin;
			newHalfedge->vertex = v;
			newHalfedge->edge = newEdge;
			newHalfedge->face = newFace;

			newHalfedgeTwin->next = previousNewHalfedge;
			newHalfedgeTwin->twin = newHalfedge;
			newHalfedgeTwin->vertex = current->vertex;
			newHalfedgeTwin->edge = newEdge;
			newHalfedgeTwin->face = previousNewFace;

			previousNewHalfedge = newHalfedge;
			previousNewFace = newFace;
		}

		halfedges[n - 1]->next = previousNewHalfedge;
		halfedges[n - 1]->face = previousNewFace;
	}
}
...
void Halfedge_Mesh::linear_subdivide() {
	...
	for (EdgeRef e = edges.begin(); e != edges.end(); e++) {
		auto h = e->halfedge;
		auto v1 = h->vertex;
		auto v2 = h->twin->vertex;
		edge_vertex_positions[e] = (v1->position + v2->position) / 2.f;
	}
	...
	for (FaceRef f = faces.begin(); f != faces.end(); f++) {
		...
		int n = 0;
		do {
			total += h->vertex->position;
			n++;
			h = h->next;
		} while (h != f->halfedge);

		face_vertex_positions[f] = total / (float)n;
	}
...
void Halfedge_Mesh::catmark_subdivide() {
	...
	for (EdgeRef e = edges.begin(); e != edges.end(); e++) {
		...
		auto v3 = face_vertex_positions[f1];
		auto v4 = face_vertex_positions[f2];

		edge_vertex_positions[e] = (v1->position + v2->position + v3 + v4) / 4.f;
	}
	...
	for (VertexRef v = vertices.begin(); v != vertices.end(); v++) {
		...
		if (v->on_boundary()) {
			auto c = h->next->vertex;
			auto a = h->twin->next->next->vertex;

			vertex_positions[v] = 0.125f * a->position + 0.125f * c->position + 0.75f * v->position;	
			continue;
		}


		int n = v->degree();
		Vec3 Q = Vec3(0.f, 0.f, 0.f);
		...
		vertex_positions[v] = (Q + 2.f * R + (n - 3.f) * v->position) / (float)n;
	}
...
bool Halfedge_Mesh::loop_subdivide() {
	...
	for (VertexRef v = vertices.begin(); v != vertices.end(); v++) {
		...
		if (v->on_boundary()) {
			...
			do {
				if (h->vertex->on_boundary() && h->vertex != v) {
					pos += h->vertex->position * 0.125f;
				}
				h = h->twin->next;
			} while (h != v->halfedge);	

			pos += v->position * 0.75f;
			vertex_new_pos[v] = pos;

			continue;
		}

		int n = v->degree();
		float u = n == 3 ? 0.1875f : (3.f / (8.f * n));
		Vec3 sumOfNeighbors = v->neighborhood_center() * n;

		vertex_new_pos[v] = (1.f - n * u) * v->position + u * sumOfNeighbors;
	}
	...
	for (EdgeRef e = edges.begin(); e != edges.end(); e++) {
		...
		if (e->on_boundary()) {
			edge_new_pos[e] = (v1->position + v2->position) / 2.f;
			continue;
		}

		edge_new_pos[e] = 0.375f * (v1->position + v2->position) + 0.125f * (h->next->next->vertex->position + h->twin->next->next->vertex->position);
	}
	...
	for (int i = 0; i < numEdges; i++) {
		...
		auto nvPos = edge_new_pos[e];
		auto nv = split_edge(e);
		if (nv) {
			auto newVertex = nv.value();
			newVertex->position = nvPos;

			auto h = newVertex->halfedge;
			auto sameEdge = h->edge;
			do {
				if (sameEdge != h->edge && !edge_new_pos.count(h->edge)) {
