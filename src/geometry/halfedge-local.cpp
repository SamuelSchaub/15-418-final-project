...
std::optional<Halfedge_Mesh::VertexRef> Halfedge_Mesh::split_edge(EdgeRef e) {
	...
	auto hNextNext = hNext->next;
	auto tNextNext = tNext->next;

	auto hVertex = hNextNext->vertex;
	auto tVertex = tNextNext->vertex;	

	auto f1 = h->face;
	auto f2 = t->face;

	auto bisectResult = bisect_edge(e);
	if (!bisectResult) {
		return std::nullopt;
	}
	auto v = bisectResult.value();

	if (!f1->boundary) {
		auto newEdge = emplace_edge();
		auto newHalfedge = emplace_halfedge();
		auto newHalfedgeTwin = emplace_halfedge();

		newEdge->halfedge = newHalfedge;

		newHalfedge->twin = newHalfedgeTwin;
		newHalfedge->next = h->next;
		newHalfedge->vertex = hVertex;
		newHalfedge->edge = newEdge;

		newHalfedgeTwin->twin = newHalfedge;
		newHalfedgeTwin->next = hNextNext;
		newHalfedgeTwin->vertex = v;
		newHalfedgeTwin->edge = newEdge;
		
		hNext->next = newHalfedge;
		h->next = newHalfedgeTwin;

		auto face = emplace_face();
		face->halfedge = newHalfedge;

		auto currentHalfedge = newHalfedge->next;
		while (currentHalfedge != newHalfedge) {
			currentHalfedge->face = face;
			currentHalfedge = currentHalfedge->next;
		}
		currentHalfedge->face = face;

		f1->halfedge = newHalfedgeTwin;
		currentHalfedge = newHalfedgeTwin->next;
		while (currentHalfedge != newHalfedgeTwin) {
			currentHalfedge->face = f1;
			currentHalfedge = currentHalfedge->next;
		}
		currentHalfedge->face = f1;
	}
	...
	if (!f2->boundary) {
		...
		newHalfedge->twin = newHalfedgeTwin;
		newHalfedge->next = t->next;
		newHalfedge->vertex = tVertex;
		newHalfedge->edge = newEdge;

		newHalfedgeTwin->twin = newHalfedge;
		newHalfedgeTwin->next = tNextNext;
		newHalfedgeTwin->vertex = v;
		newHalfedgeTwin->edge = newEdge;
		
		tNext->next = newHalfedge;
		t->next = newHalfedgeTwin;
		...
		f2->halfedge = newHalfedgeTwin;
		currentHalfedge = newHalfedgeTwin->next;
		while (currentHalfedge != newHalfedgeTwin) {
			currentHalfedge->face = f2;
			currentHalfedge = currentHalfedge->next;
		}
		currentHalfedge->face = f2;
	}
...
std::optional<Halfedge_Mesh::FaceRef> Halfedge_Mesh::extrude_face(FaceRef f) {
	...
	auto hNext = h->next;
    auto vH = h->vertex;
    auto vHNext = hNext->vertex;

    auto newVH = emplace_vertex();
    auto newVHNext = emplace_vertex();
    auto newEdge = emplace_edge();
    auto newHalfedge = emplace_halfedge();
    auto newHalfedgeTwin = emplace_halfedge();
    ...
    newVH->halfedge = newHalfedge;
    newVH->position = vH->position;
    newVHNext->position = vHNext->position;

    newHalfedge->vertex = newVH;
    newHalfedge->twin = newHalfedgeTwin;
    newHalfedge->edge = newEdge;
    newHalfedge->face = f;

    newHalfedgeTwin->vertex = newVHNext;
    newHalfedgeTwin->twin = newHalfedge;
    newHalfedgeTwin->edge = newEdge;
    newHalfedgeTwin->face = f;

    f->halfedge = newHalfedge;

    int totalVertices = 2;
    auto previousVertex = newVHNext;
    auto previousHalfedge = newHalfedge;
    auto currentHalfedge = hNext->next;

    do {
        auto newVertex = emplace_vertex();
        auto previousToNewHalfedge = emplace_halfedge();
        auto previousToNewHalfedgeTwin = emplace_halfedge();
        auto previousToNewEdge = emplace_edge();

        newVertex->position = currentHalfedge->vertex->position;
        totalVertices++;

        previousToNewEdge->halfedge = previousToNewHalfedge;

        previousToNewHalfedge->twin = previousToNewHalfedgeTwin;
        previousToNewHalfedge->edge = previousToNewEdge;
        previousToNewHalfedge->vertex = previousVertex;
        previousToNewHalfedge->face = f;

        previousToNewHalfedgeTwin->next = previousHalfedge->twin;
        previousToNewHalfedgeTwin->twin = previousToNewHalfedge;
        previousToNewHalfedgeTwin->edge = previousToNewEdge;
        previousToNewHalfedgeTwin->vertex = newVertex;
        previousToNewHalfedgeTwin->face = f;

        previousVertex->halfedge = previousToNewHalfedge;
        previousHalfedge->next = previousToNewHalfedge;
        previousVertex = newVertex;
        previousHalfedge = previousToNewHalfedge;

        currentHalfedge = currentHalfedge->next;
    } while (currentHalfedge != h);

	auto lastEdge = emplace_edge();
	auto lastHalfedge = emplace_halfedge();
	auto lastHalfedgeTwin = emplace_halfedge();

	lastEdge->halfedge = lastHalfedge;

	lastHalfedge->twin = lastHalfedgeTwin;
	lastHalfedge->next = newHalfedge;
	lastHalfedge->vertex = previousVertex;
	lastHalfedge->edge = lastEdge;
	lastHalfedge->face = f;

	lastHalfedgeTwin->twin = lastHalfedge;
	lastHalfedgeTwin->next = previousHalfedge->twin;
	lastHalfedgeTwin->vertex = newVH;
	lastHalfedgeTwin->edge = lastEdge;
	lastHalfedgeTwin->face = f;

	previousVertex->halfedge = lastHalfedge;
	previousHalfedge->next = lastHalfedge;
	newHalfedgeTwin->next = lastHalfedgeTwin;

    currentHalfedge = h;
    auto currentPreviousHalfedge = currentHalfedge->next;
    while (currentPreviousHalfedge->next != h) {
		currentPreviousHalfedge = currentPreviousHalfedge->next;
	}

    previousHalfedge = newHalfedge;
	for (int i = 0; i < totalVertices; i++) {
        auto currentNewEdge = emplace_edge();
        auto currentNewHalfedge = emplace_halfedge();
        auto currentNewHalfedgeTwin = emplace_halfedge();

        currentNewEdge->halfedge = currentNewHalfedge;

        currentNewHalfedge->twin = currentNewHalfedgeTwin;
        currentNewHalfedge->next = currentHalfedge;
        currentNewHalfedge->edge = currentNewEdge;
        currentNewHalfedge->vertex = previousHalfedge->vertex;

        currentNewHalfedgeTwin->twin = currentNewHalfedge;
        currentNewHalfedgeTwin->next = previousHalfedge->twin->next;
        currentNewHalfedgeTwin->edge = currentNewEdge;
        currentNewHalfedgeTwin->vertex = currentHalfedge->vertex;

        previousHalfedge->twin->next = currentNewHalfedge;
        currentPreviousHalfedge->next = currentNewHalfedgeTwin;

        currentPreviousHalfedge = currentHalfedge;
        currentHalfedge = currentHalfedge->next;
    ...
    do {
        auto newFace = emplace_face();
        auto currentHalfedgeTwin = currentHalfedge->twin;
        newFace->halfedge = currentHalfedgeTwin;
		currentHalfedgeTwin->face = newFace;

		currentHalfedgeTwin = currentHalfedgeTwin->next;
		while (currentHalfedgeTwin != currentHalfedge->twin) {
			currentHalfedgeTwin->face = newFace;
			currentHalfedgeTwin = currentHalfedgeTwin->next;
		}
...
std::optional<Halfedge_Mesh::EdgeRef> Halfedge_Mesh::flip_edge(EdgeRef e) {
	...
	auto hNextNext = hNext->next;
	auto tNextNext = tNext->next;
	auto hPrevious = hNext->next;
	auto tPrevious = tNext->next;

	while (hPrevious->next != h) {
		hPrevious = hPrevious->next;
	}

	while (tPrevious->next != t) {
		tPrevious = tPrevious->next;
	}

	auto f1 = h->face;
	auto f2 = t->face;

	auto v1 = hNext->vertex;
	auto v2 = tNext->vertex;
	auto v3 = hNextNext->vertex;
	auto v4 = tNextNext->vertex;
	...
	tPrevious->next = hNext;
	hPrevious->next = tNext;
...
std::optional<Halfedge_Mesh::VertexRef> Halfedge_Mesh::collapse_edge(EdgeRef e) {
	...
	auto hNextTwin = hNext->twin;
	auto tNextTwin = tNext->twin;

	auto hNextNext = hNext->next;
	auto tNextNext = tNext->next;

	auto hNextNextTwin = hNextNext->twin;
	auto tNextNextTwin = tNextNext->twin;

	auto hPrevious = h->next;
	while (hPrevious->next != h) {
		hPrevious = hPrevious->next;
	}

	auto tPrevious = t->next;
	while (tPrevious->next != t) {
		tPrevious = tPrevious->next;
	}
	...
	vT->position = (vH->position + vT->position) / 2.0f;

	current = vH->halfedge;
	current->vertex = vT;
	current = current->twin->next;
	while (current != vH->halfedge) {
		current->vertex = vT;
		current = current->twin->next;
	}
	...
	hNext->vertex = vT;
	h->face->halfedge = hNext;

	vT->halfedge = t->next;

	if (numEdgesH == 3) {
		hNextTwin->twin = hNextNextTwin;
		hNextNextTwin->twin = hNextTwin;
		hNextNextTwin->edge = hNext->edge;
		hNext->edge->halfedge = hNext->twin;
		hNextNext->edge->halfedge = hNextNextTwin;
		vT->halfedge = hNextNextTwin;
		hNextNext->vertex->halfedge = hNextTwin;

		erase_face(hNext->face);
		erase_edge(hNextNext->edge);
		erase_halfedge(hNext);
		erase_halfedge(hNextNext);
	}

	tPrevious->next = tNext;
	tNext->vertex = vT;
	t->face->halfedge = tNext;


	if (numEdgesT == 3) {
		tNextTwin->twin = tNextNextTwin;
		tNextNextTwin->twin = tNextTwin;
		tNextNextTwin->edge = tNext->edge;
		tNext->edge->halfedge = tNext->twin;
		tNextNext->edge->halfedge = tNextNextTwin;
		vT->halfedge = tNextNextTwin;
		tNextNext->vertex->halfedge = tNextTwin;

		erase_face(tNext->face);
		erase_halfedge(tNext);
		erase_halfedge(tNextNext);
	}
...
void Halfedge_Mesh::extrude_positions(FaceRef face, Vec3 move, float shrink) {
	...
	auto centroid = face->center();
	auto h = face->halfedge;
	auto current = h->next;
	while (current != h) {
		current->vertex->position = (1.0f - shrink) * current->vertex->position + shrink * centroid + move;
		current = current->next;
	}
	current->vertex->position = (1.0f - shrink) * current->vertex->position + shrink * centroid + move;
}
