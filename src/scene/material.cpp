...
Vec3 refract(Vec3 out_dir, float index_of_refraction, bool& was_internal) {
	...
	auto cosThetaT = out_dir.y;

	float sin2ThetaT = std::max(0.0f, 1.0f - cosThetaT * cosThetaT);
	float sin2ThetaI = index_of_refraction * index_of_refraction * sin2ThetaT;
	if (sin2ThetaI >= 1.0f) {
		was_internal = true;
		return Vec3{};
	}

	float cosThetaI = std::sqrt(1 - sin2ThetaI);
	was_internal = false;
	if (out_dir.y > 0) {
		return index_of_refraction * -out_dir + (index_of_refraction * cosThetaT - cosThetaI) * Vec3{0.0f, 1.0f, 0.0f};
	} else {
		return index_of_refraction * -out_dir + (index_of_refraction * cosThetaT + cosThetaI) * Vec3{0.0f, 1.0f, 0.0f};	
	}
}
...
float schlick(Vec3 in_dir, float index_of_refraction) {
	...
	auto r0 = ((1.0f - index_of_refraction) * (1.0f - index_of_refraction)) / ((1.0f + index_of_refraction) * (1.0f + index_of_refraction));
	auto oneMinusCosTheta = 1 - std::abs(in_dir.y);
...
Spectrum Lambertian::evaluate(Vec3 out, Vec3 in, Vec2 uv) const {
	...
	auto albedoValue = albedo.lock()->evaluate(uv) / PI_F;
	auto cosTheta = std::max(0.0f, in.y);

    return albedoValue * cosTheta;
}
...
Scatter Refract::scatter(RNG &rng, Vec3 out, Vec2 uv) const {
	...
	bool internal = false;
	auto dir = refract(out, eta, internal);
	// auto dirSpectrum = Spectrum(dir);
	auto atten = eta * eta * transmittance.lock()->evaluate(uv);

	if (internal) {
		dir = reflect(out);
		// atten = Spectrum{1, 1, 1};
	}

    Scatter ret;
    ret.direction = dir;
    ret.attenuation = atten;
    return ret;
}
...
Scatter Glass::scatter(RNG &rng, Vec3 out, Vec2 uv) const {
	...
	auto reflectProb = schlick(out, eta);
	if (rng.coin_flip(reflectProb)) {
		Scatter ret;
		ret.direction = reflect(out);
		ret.attenuation = reflectance.lock()->evaluate(uv);
		return ret;
