...
std::pair<Ray, float> Camera::sample_ray(RNG &rng, uint32_t px, uint32_t py) {
	...
	float height = 2.0f * std::tan(Radians(vertical_fov) / 2.0f);
	float width = height * aspect_ratio;
	float x_cord = sensor_pixel.x * width / film.width - width / 2.0f;
	float y_cord = sensor_pixel.y * height / film.height - height / 2.0f;

	//Build ray:
	Ray ray;
	ray.point = Vec3(); //ray should start at the origin
	ray.dir = Vec3(x_cord, y_cord, -1.0f); //TODO: compute from sensor plane position
