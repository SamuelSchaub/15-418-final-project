...
private:
	...
	BBox calculateBbox(size_t left, size_t right);
	float calculateSAH(size_t left, size_t right, int axis, float split, float sc);
	size_t buildHelper(size_t start, size_t end, size_t max_leaf_size);
	void hitHelper(const Ray& ray, size_t nodeIndex, Trace& best) const;
};
