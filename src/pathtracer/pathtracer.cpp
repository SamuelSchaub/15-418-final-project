...
Spectrum Pathtracer::sample_direct_lighting_task4(RNG &rng, const Shading_Info& hit) {
	...
	Materials::Scatter bsdfScatter = hit.bsdf.scatter(rng, hit.out_dir, hit.uv);

	//TODO: rotate that direction into world coordinates
	Vec3 worldDir = hit.object_to_world.rotate(bsdfScatter.direction);

	//TODO: construct a ray traveling in that direction
	// NOTE: because we want emitted light only, can use depth = 0 for the ray
	Ray ray = Ray(hit.pos, worldDir, Vec2{EPS_F, std::numeric_limits<float>::max()}, 0);
	...
	Spectrum directLight = emissive * bsdfScatter.attenuation;
	if (!hit.bsdf.is_specular()) {
		directLight = directLight / hit.bsdf.pdf(hit.out_dir, bsdfScatter.direction);
	}

	return radiance + directLight;
}
...
Spectrum Pathtracer::sample_direct_lighting_task6(RNG &rng, const Shading_Info& hit) {
	...
	if (hit.bsdf.is_specular()) {
		auto bsdfScatter = hit.bsdf.scatter(rng, hit.out_dir, hit.uv);
		auto worldDir = hit.object_to_world.rotate(bsdfScatter.direction);
		auto ray = Ray(hit.pos, worldDir, Vec2{EPS_F, std::numeric_limits<float>::max()}, 0);
		auto [emissive, light] = trace(rng, ray);
		radiance += emissive * bsdfScatter.attenuation;
		return radiance;
	}
	...
	if (rng.coin_flip(0.5f)) {
		// material sampling
		auto bsdfScatter = hit.bsdf.scatter(rng, hit.out_dir, hit.uv);
		localDir = bsdfScatter.direction;
		worldDir = hit.object_to_world.rotate(localDir);
	} else {
		// area light sampling
		worldDir = sample_area_lights(rng, hit.pos);
		localDir = hit.world_to_object.rotate(worldDir);
	}

	auto ray = Ray(hit.pos, worldDir, Vec2{EPS_F, std::numeric_limits<float>::max()}, 0);
	auto [emissive, light] = trace(rng, ray);
	auto materialPDF = hit.bsdf.pdf(hit.out_dir, localDir);
	auto areaLightPDF = area_lights_pdf(hit.pos, worldDir);
	auto attenuation = hit.bsdf.evaluate(hit.out_dir, localDir, hit.uv);
	auto pdf = (materialPDF + areaLightPDF) * 0.5f;
	radiance += emissive * attenuation / pdf;
...
Spectrum Pathtracer::sample_indirect_lighting(RNG &rng, const Shading_Info& hit) {
	...
	auto ray = Ray(hit.pos, worldDir, Vec2{EPS_F, std::numeric_limits<float>::max()}, hit.depth - 1);
	...
	auto indirectLight = light * bsdfScatter.attenuation;
	if (!hit.bsdf.is_specular()) {
		indirectLight = indirectLight / hit.bsdf.pdf(hit.out_dir, bsdfScatter.direction);
	}
