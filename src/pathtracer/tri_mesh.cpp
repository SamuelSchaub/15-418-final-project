...
BBox Triangle::bbox() const {
    ...
    float xMin = std::min(v_0.position.x, std::min(v_1.position.x, v_2.position.x));
	float yMin = std::min(v_0.position.y, std::min(v_1.position.y, v_2.position.y));
	float zMin = std::min(v_0.position.z, std::min(v_1.position.z, v_2.position.z));
	float xMax = std::max(v_0.position.x, std::max(v_1.position.x, v_2.position.x));
	float yMax = std::max(v_0.position.y, std::max(v_1.position.y, v_2.position.y));
	float zMax = std::max(v_0.position.z, std::max(v_1.position.z, v_2.position.z));
...
Trace Triangle::hit(const Ray& ray) const {
	...
	// auto denominator = dot(cross(e2, ray.dir), e2);
	auto crossVec = cross(ray.dir, e2);
	auto denominator = dot(e1, crossVec);
	if (denominator == 0.0f) {
		return Trace();
	}

	float inv = 1 / denominator;
	float u = dot(s, crossVec) * inv;
	if (u < 0 || u > 1) {
		return Trace();
	}

	auto crossVec2 = cross(s, e1);
	float v = dot(ray.dir, crossVec2) * inv;
	if (v < 0 || u + v > 1) {
		return Trace();
	}

	float t = dot(e2, crossVec2) * inv;
